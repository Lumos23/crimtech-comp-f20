...
class Snake(object):
    ...
    def move(self):
        ...
        # check if there's a self collision
        if self.collision(newHead[0], newHead[1]):
            self.kill()
...
def main():
    ...
    while True:
        ...
        if snake.get_head() == apple.position:
            ...
            # add length to the front
            front = tuple(map(sum, zip(snake.body[0], DIR.get(snake.direction))))
            snake.body = [front] + snake.body
            apple.place(snake)
            snake.l += 1
            score += 1
        ...
        screen.blit(font.render("Score: " + str(score), 1, (10, 10, 10)), (0, 0))
